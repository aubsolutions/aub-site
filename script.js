...
const copy = {
  ru: {
    ...
    formOverline: 'ОБРАТНАЯ СВЯЗЬ', formTitle: 'Расскажите о вашем проекте', formIntro: 'Заполните поля — мы подготовим письмо, которое останется отправить из вашей почты.', formLabels: ['Ваше имя', 'Телефон или email', 'Кратко о задаче'], formSubmit: 'Подготовить письмо', close: 'Закрыть', status: 'Открываем почтовое приложение…', subject: 'Новая заявка с сайта AUB Solutions', mailFields: ['Имя', 'Контакт', 'Задача']
  },
  kk: {
    ...
    formOverline: 'КЕРІ БАЙЛАНЫС', formTitle: 'Жобаңыз туралы айтып беріңіз', formIntro: 'Жолдарды толтырыңыз — біз поштаңыздан жіберуге дайын хат жасаймыз.', formLabels: ['Атыңыз', 'Телефон немесе email', 'Міндет туралы қысқаша'], formSubmit: 'Хат дайындау', close: 'Жабу', status: 'Пошта қолданбасын ашып жатырмыз…', subject: 'AUB Solutions сайтынан жаңа өтінім', mailFields: ['Аты', 'Байланыс', 'Міндет']
  },
  en: {
    ...
    contactTitle: 'Ready to discuss your project?', contactText: 'Send a request and we will contact you to discuss the task and propose the right solution.', footerTagline: 'Technology. People. Results.', rights: 'All rights reserved.', footerEnd: 'Development. Implementation. Support.', formOverline: 'GET IN TOUCH', formTitle: 'Tell us about your project', formIntro: 'Complete the fields and we will prepare an email for you to send from your mail app.', formLabels: ['Your name', 'Phone or email', 'Briefly describe the task'], formSubmit: 'Prepare email', close: 'Close', status: 'Opening your email app…', subject: 'New enquiry from the AUB Solutions website', mailFields: ['Name', 'Contact', 'Task']
  }
};
...
contactForm?.addEventListener('submit', (event) => {
  ...
  const body = `${t.mailFields[0]}: ${data.get('name')}\n${t.mailFields[1]}: ${data.get('contact')}\n\n${t.mailFields[2]}:\n${data.get('message')}`;
  setText('.form-status', t.status); window.location.href = `mailto:u.assylbekov@gmail.com?subject=${encodeURIComponent(t.subject)}&body=${encodeURIComponent(body)}`;
});
